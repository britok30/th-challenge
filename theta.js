...
const coinChange = (coins, amount) => {
    // minimumCoins[i] represents the least amount of coins that can make the value equals to the i
    const max = Number.MAX_VALUE || Infinity;
    const minimumCoins = Array(amount + 1).fill(max);
    minimumCoins[0] = 0;
    for (let i = 1; i <= amount; i++) {
        for (const coin of coins) {
            if (i - coin >= 0) {
                minimumCoins[i] = Math.min(
                    minimumCoins[i],
                    minimumCoins[i - coin] + 1
                );
            }
        }
    }
    return console.log(
        minimumCoins[amount] === max ? -1 : minimumCoins[amount]
    );
};
